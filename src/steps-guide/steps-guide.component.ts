...
export class StepsGuideComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  updatePosition() {
    ...
    switch (rect.placementPrimary) {
      case 'top':
        left = targetRect.left + this.triggerElement.clientWidth / 2 - this.elm.nativeElement.clientWidth / 2 + this.SUN_WIDTH;
        top = top + this.triggerElement.clientHeight / 2 - this.DOT_HORIZONTAL_MARGIN - (rect.placementSecondary == 'center'?this.SUN_HEIGHT:0);
        break;
      case 'bottom':
        left = targetRect.left + this.triggerElement.clientWidth / 2 - this.elm.nativeElement.clientWidth / 2;
        top = top + this.DOT_HORIZONTAL_MARGIN / 2 + (rect.placementSecondary == 'center' ? this.SUN_HEIGHT + 20 : 0);
        break;
      case 'left':
        left = left + this.triggerElement.clientWidth / 2 - this.DOT_HORIZONTAL_MARGIN - this.SUN_WIDTH;
    ...
    switch (rect.placementSecondary) {
      case 'left':
        ...
        break;
      case 'right':
        left = targetRect.left - this.elm.nativeElement.clientWidth + this.triggerElement.clientWidth + this.SUN_WIDTH;
        break;
      default:
    }
